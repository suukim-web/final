...
if (playButton && detailAudio) {
  ...
  const songData = window.songData || {
    ...
    "thewayshedances": {
      ...
      text: `
        ...
        <span class="en">Pharrell Williams, Chad Hugo, Shay Haley</span>로 구성된 그룹으로, 록, 힙합, 펑크<span class="en">(Funk), R&B</span>를 자유롭게 결합한 음악으로 알려져 있다.
        <p>
  ...
  function openPanelByTrack(track) {
    const key = track.dataset.song;
    const data = songData[key];

    if (!data) {
      console.warn("songData 없음:", key);
      return;
    }
...
if (data.overlayClass) {
  ...
  let virtualScroll = 0;
let maxVirtualScroll = Math.max(1, tracks.length - 1);
let isPanelOpen = false;

  function layoutTracks() {
    const vinyl = document.querySelector(".big-vinyl");
    if (!vinyl || tracks.length === 0) return;
...
const progress = virtualScroll / maxVirtualScroll;
    ...
    const focusIndex = Math.round(progress * (tracks.length - 1));
    const centerIndex = progress * (tracks.length - 1);
    ...
    tracks.forEach((track, i) => {
      ...
      const rotate = angle + 12;

      track.style.left = `${x}px`;
      track.style.top = `${y}px`;
      track.style.transform = `translate(-50%, -50%) rotate(${rotate}deg)`;

      track.classList.toggle("is-focus", i === focusIndex);
    });

if (focusIndex !== currentFocusedIndex) {
  currentFocusedIndex = focusIndex;

  if (isPanelOpen) {
    openPanelByTrack(tracks[focusIndex]);
  }
}}
